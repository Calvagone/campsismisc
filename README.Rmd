...
```{r sensibility_analysis_auc_example , fig.align='center', fig.height=3, fig.width=6, message=F}
dataset <- Dataset(1) %>%
...
```{r sensibility_analysis_auc_example_absolute , fig.align='center', fig.height=3, fig.width=6, message=F}
object %>% getTornadoPlot(relative=FALSE)
```

Let's do the same exercise on Cmax.

```{r sensibility_analysis_cmax_example , fig.align='center', fig.height=3, fig.width=6, message=F}
dataset <- Dataset(1) %>%
  add(Infusion(time=0, amount=1000, compartment=1)) %>%
  add(Observations(times=0:24)) %>%
  add(Covariate("METAB", 0)) %>%
